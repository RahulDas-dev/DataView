 ...
 */
export function duplicated(df: DataFrame, options?: { subset?: string[], keep?: 'first' | 'last' | false }): Series {
    ...
    const dataToCheck = df.values.map(row => subset.map(col => row[df.columns.indexOf(col)]));
